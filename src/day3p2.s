...
.main:  # a0: input fd
    ...
    sd      s4, 40(sp)  # line length
    sd      s5, 48(sp)  # search start
    sd      s6, 56(sp)  # curr battery value
    sd      s7, 56(sp)  # battery sum
    sd      s8, 64(sp)  # remaining battery length
...
.main.process_line.battery_digits:
    add     a0, s2, s5
    sub     a1, s4, s8
    sub     a1, a1, s5
    addi    a1, a1, 1
    ...
    addi    s8, s8, -1
    bgtz    s8, .main.process_line.battery_digits
