...
.main:  # a0: input fd
    ...
    sd      s3, 4*8(sp) # removed count
    sd      s4, 5*8(sp) # prev removed count
    sd      s5, 6*8(sp) # 
    sd      s6, 7*8(sp) #
    sd      s7, 8*8(sp) #
    sd      s8, 9*8(sp) #
    ...
    j       .main.outer_loop.body

.main.outer_loop.continue:
    beq     s3, s4, .main.end
.main.outer_loop.body:
    mv      s4, s3
    mv      a3, zero
    mv      a4, zero
    j       .main.remove_accessible.loop_body

.main.remove_accessible.loop_col:
    addi    a3, a3, 1
    blt     a3, a1, .main.remove_accessible.loop_body
.main.remove_accessible.loop_row:
    mv      a3, zero
    addi    a4, a4, 1
    bge     a4, a2, .main.outer_loop.continue
.main.remove_accessible.loop_body:
    call    .check_cell_accessible
    beqz    a5, .main.remove_accessible.loop_col
    addi    s3, s3, 1
    call    .compute_map_index
    li      t0, '.'
    sb      t0, (a5)
    j       .main.remove_accessible.loop_col
...
.get_cell_occupied.ret:
    ld      ra, -0*8(sp)
    addi    sp, sp, 1*8
    ret
.get_cell_occupied.out_of_bounds:
    li      a5, 0
    j       .get_cell_occupied.ret
